...
internal class GetWeatherForecastUseCase @Inject constructor(private var weatherRepository: WeatherRepository)  {
    suspend operator fun invoke(): DomainResult<ModelWeather> = weatherRepository.getForecast()//DomainResult.Success(ModelWeather(0.1,0.2))//weatherRepository.getDetail()
}